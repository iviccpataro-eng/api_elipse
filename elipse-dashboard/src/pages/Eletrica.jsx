...
export default function Eletrica() {
    ...
    return (
        <div className="flex min-h-screen bg-gray-50">
            ...
            {/* Conteúdo principal */}
            <main className="flex-1 p-6 overflow-y-auto">{renderEquipamentos()}
                <button
                    key={eq}
                    onClick={() => handleEquipamentoClick(tag)}
                    className="w-full flex items-center gap-3 border rounded-xl p-4 bg-white shadow hover:shadow-md transition text-left"
                >
                    <Gauge className="w-5 h-5 text-blue-600 flex-shrink-0" />
                    <span className="font-medium text-blue-700">{info.name || info.equipamento || eq}</span>
                </button>
            </main>
        </div>
    );
